...
`doctor` only reports **local key presence**. It does not send a request and must never be interpreted as API authentication, entitlement, billing, or subscription verification.
...
4. Only if the caller has deliberately supplied `OPENAI_API_KEY`, invoke `generate --json` once.
...
- **No subscription shortcut:** do not pass Codex/ChatGPT session cookies or access tokens. The documented Image API expects API billing credentials.
